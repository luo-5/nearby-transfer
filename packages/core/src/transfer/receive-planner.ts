/**
 * Receive target planner: reserves conflict-free top-level target names and
 * builds the staging/final path map for an incoming transfer.
 * Ported from src/v2/receive-target-planner.js.
 *
 * The planner creates a per-task staging directory under the receive root and a
 * shared reservation directory that records which top-level names a task has
 * claimed. Names that already exist on disk (case-insensitively on Windows) or
 * that another in-flight task has reserved are auto-renamed with a numeric
 * suffix, so concurrent receives never clobber each other or existing data.
 */
...
import { assertValidTaskId, normalizeTransferManifest, type TransferManifest, type ManifestEntry } from './manifest.js';

export const STAGING_PREFIX = '.nearby-transfer-staging-';
export const STAGING_SUFFIX = '.partial';
export const RESERVATION_ROOT_NAME = '.nearby-transfer-reservations';
...
interface PlanReceiveTargetsInput {
  readonly fsPromises?: FsPromises;
  readonly manifest: unknown;
  readonly receiveRoot: unknown;
}

interface CleanupReceiveStagingInput {
  readonly fsPromises?: FsPromises;
  readonly receiveRoot: unknown;
...
interface ReceiveTargetPlan {
  readonly taskId: string;
  readonly receiveRoot: string;
  readonly stagingDirectory: string;
  readonly targets: readonly ReceiveTarget[];
}

interface ReceiveStagingCleanupResult {
  readonly stagingRemoved: boolean;
  readonly reservationsReleased: number;
}

interface TopLevelRoot {
...
export async function planReceiveTargets(input: PlanReceiveTargetsInput): Promise<ReceiveTargetPlan> {
  assertPlainObject(input, 'Receive target planner input');
  assertOnlyKeys(input as unknown as Record<string, unknown>, PLAN_INPUT_KEYS, 'Receive target planner input');
  requireOwn(input as unknown as Record<string, unknown>, 'manifest', 'Receive target planner input');
  requireOwn(input as unknown as Record<string, unknown>, 'receiveRoot', 'Receive target planner input');
  ...
  try {
    ...
    for (const root of roots) {
      ...
      let suffix = 0;
      let reserved: ReservationResult | null | undefined;
      while (!reserved) {
    ...
    const targets = manifest.entries.map((entry): ReceiveTarget => {
      const components = entry.path.split('/');
      const finalRoot = rootMappings.get(components[0]!)!;
      const finalComponents = [finalRoot, ...components.slice(1)];
...
export async function cleanupReceiveStaging(input: CleanupReceiveStagingInput): Promise<ReceiveStagingCleanupResult> {
  assertPlainObject(input, 'Receive staging cleanup input');
  assertOnlyKeys(input as unknown as Record<string, unknown>, CLEANUP_INPUT_KEYS, 'Receive staging cleanup input');
  requireOwn(input as unknown as Record<string, unknown>, 'receiveRoot', 'Receive staging cleanup input');
  requireOwn(input as unknown as Record<string, unknown>, 'taskId', 'Receive staging cleanup input');
  ...
  const stagingDirectory = path.join(
    receiveRoot,
    `${STAGING_PREFIX}${taskId}${STAGING_SUFFIX}`,
  );
  assertContained(receiveRoot, stagingDirectory, 'Task staging directory');
...
function collectTopLevelRoots(manifest: TransferManifest): TopLevelRoot[] {
  const roots = new Map<string, TopLevelRoot>();
...
async function createStagingDirectories(targets: readonly ReceiveTarget[], stagingDirectory: string, fsPromises: FsPromises): Promise<void> {
  const directories = new Set<string>([stagingDirectory]);
...
async function ensureSafeDirectory(directory: string, fsPromises: FsPromises, subject: string): Promise<void> {
  try {
...
async function assertSafeDirectoryChain(directory: string, fsPromises: FsPromises, subject: string): Promise<void> {
...
async function assertSafeDirectory(directory: string, fsPromises: FsPromises, subject: string): Promise<void> {
...
async function assertSafeTree(target: string, fsPromises: FsPromises): Promise<void> {
...
async function releaseTaskReservations(receiveRoot: string, taskId: string, fsPromises: FsPromises): Promise<number> {
  const reservationRoot = path.join(receiveRoot, RESERVATION_ROOT_NAME);
  ...
  try {
    ...
    // ENOENT: directory gone. ENOTEMPTY/EEXIST: another reservation appeared.
    // EPERM/EACCES: on Windows a concurrent reservation cleanup or antivirus
    // scan can transiently block rmdir; treat as recoverable and leave the
    // directory for a later cleanup rather than failing the transfer.
    const tolerated = ['ENOENT', 'ENOTEMPTY', 'EEXIST', 'EPERM', 'EACCES'];
    if (!error || !tolerated.includes((error as NodeJS.ErrnoException).code ?? '')) throw error;
  }
  return released;
}

async function removeOwnedReservation(reservationDirectory: string, taskId: string, fsPromises: FsPromises): Promise<boolean> {
  ...
  try {
    ...
    if (error && (error as NodeJS.ErrnoException).code === 'ENOENT') return false;
    throw error;
  }
}

async function readdirIfExists(directory: string, fsPromises: FsPromises): Promise<string[] | null> {
  try {
    ...
    if (error && ['ENOENT', 'EPERM', 'EACCES'].includes((error as NodeJS.ErrnoException).code ?? '')) return null;
    throw error;
  }
}

async function hasWindowsEntry(directory: string, candidate: string, fsPromises: FsPromises): Promise<boolean> {
  const candidateKey = windowsKey(candidate);
  const names = await fsPromises.readdir(directory);
  return names.some((name) => windowsKey(name) === candidateKey);
}

function createRenamedComponent(component: string, kind: ManifestEntry['kind'], suffixNumber: number): string {
...
function normalizeAbsoluteRoot(receiveRoot: unknown): string {
...
function normalizeFsPromises(value: FsPromises | undefined): FsPromises {
  ...
  for (const method of ['lstat', 'mkdir', 'readdir', 'rename', 'rm', 'rmdir']) {
    if (typeof (candidate as unknown as Record<string, unknown>)[method] !== 'function') {
...
function assertContained(root: string, candidate: string, subject: string): void {
...
async function lstatIfExists(target: string, fsPromises: FsPromises): Promise<fs.Stats | null> {
  try {
...
async function pathExists(target: string, fsPromises: FsPromises): Promise<boolean> {
...
function requireOwn(value: Record<string, unknown>, key: string, subject: string): void {
